...
public class IndexController {
     ...
     * @return
     */
    @GetMapping({"/index/esSearch"})
    public String elasticSearchIndexPage(HttpServletRequest request,
                            @RequestParam(value = "categoryId", required = false) Long categoryId,
                            @RequestParam(value = "page", required = false, defaultValue = "1") Integer page,
                            @RequestParam(value = "keyword", required = false) String keyword,
                            @RequestParam(value = "period", required = false) String period,
        ...
        //第三步：将帖子列表数据封装到request域中
//        request.setAttribute("pageResult", postService.getPostPageForIndex(pageUtil));
        request.setAttribute("pageResult", postService.getPostPageByElasticSearchForIndex(pageUtil));
        return "index";
    }
